-- =====================================================
-- Neo's Burritos - Database Triggers
-- Audit trails and data integrity enforcement
-- =====================================================
...
BEGIN
    IF OLD.status != NEW.status THEN
        INSERT INTO transaction_log (order_id, payment_method, payment_status, amount, transaction_reference, processed_at)
        VALUES (NEW.order_id, 'CASH', 'PENDING', NEW.total_amount, 
...
-- Prevent direct modification of sensitive user data
CREATE TRIGGER tr_prevent_role_change
BEFORE UPDATE ON users
FOR EACH ROW
BEGIN
    -- Only allow role changes by admin (this would be enforced at application level)
    -- For now, we'll log any role changes
    IF OLD.role != NEW.role THEN
        INSERT INTO transaction_log (order_id, payment_method, payment_status, amount, transaction_reference, processed_at)
        VALUES (0, 'CASH', 'COMPLETED', 0, 
...
FOR EACH ROW
BEGIN
    -- This will be validated at application level since we can't access cart during order creation
    -- But we can ensure minimum order amount
...
-- =====================================================
-- CURRENCY UPDATE TRIGGERS
-- =====================================================

-- Log currency rate changes for audit
...
BEGIN
    IF OLD.exchange_rate_to_usd != NEW.exchange_rate_to_usd THEN
        INSERT INTO transaction_log (order_id, payment_method, payment_status, amount, transaction_reference, processed_at)
        VALUES (0, 'CASH', 'COMPLETED', NEW.exchange_rate_to_usd, 
